...
\let\Bbbk\undefined % Undefines some symbols for amssymb
\usepackage{amssymb}
...
Both the piecewise constant and linear approximation implementations that we  presented in codes~\ref{code:piecewise_constant_approximation} and \ref{code:piecewise_linear_approximation} are non branching, vector capable, and use only basic arithmetic and bit wise operations, and thus we anticipate their performance should be exceptionally good. Indeed, their performance, along with several other implementations are shown in table~\ref{tab:implementation_times}, with experiments performed on an Intel Xeon Gold (Skylake) CPU and an Arm based Cavium ThunderX2 \citep{sheridan2020approximate_random}.\todo{Perhaps Simon McIntosh smith has some new hardware where we can test out the piecewise polynomials using OpenMO and assembly?}
...
\hfill
\subfigure[A piecewise linear approximation to the non central $ \chi^2 $ distribution with $ \nu = 1 $ and $ \lambda \in \{1,10,20\} $.\label{fig:non_central_chi_squared_linear_approximation}]{\includegraphics{non_central_chi_squared_linear_approximation}}\hfill
...
\subsection{Simulating paths from the Cox-Ingersoll-Ross process}
...
\todo[inline=true, caption={Conclusions}]{
\begin{enumerate}
\item Conclusions (mimic conclusions)
\begin{enumerate}
\item What problem have we solved/tackled. (random variables are expensive). 
\item How did we solve it? (Inverse transform method with approximations, piecewise constant, linear, and higher order piecewise polynomials).
\item How are our solutions interesting theoretically and why are they important? (nested MLMC analysis, ans slight improvement on ritter.)
\item Why do our implementations work so especially well? (non branching, simple operations, exploit the cache or vector registers.)
\item What savings can a user expect?
\item It works for the non central $ \chi^2 $ distribution too! (which already struggles with large bias).
\item Where can i get hold of these implementations? (My repos)
\end{enumerate}
\end{enumerate}
}
