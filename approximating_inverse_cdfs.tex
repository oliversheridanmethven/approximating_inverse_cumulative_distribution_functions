...
\subfigure[The variance of the terms from simulating the CIR process. The (\raisebox{-0.1em}{\huge$ \bullet$})-marker is the underlying CIR process, and the markers ({\large $ \blacktriangledown $}, $ \blacksquare $, $ \blacklozenge $) indicate the variances of possible two way multilevel corrections. ({\large $ \blacktriangledown $}) Euler-Maruyama. ($ \blacksquare $) Piecewise linear. ($ \blacklozenge $) Piecewise cubic.\label{fig:cir_process_variance_reduction}]{\includegraphics{variance_reduction_cir_process}}\hfil
